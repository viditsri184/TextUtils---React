...
export default function TextForm(props) {
    ...
    return (
        ...
        <div className={`container my-3 text-${props.mode==='light' ? 'dark' : 'light'}`}>
            ...
            <button disabled={text.length === 0} className="btn btn-primary mx-2 my-1" onClick={handleCopyClick}>Copy Text to Clipboard</button>
        ...
        <div className={`container my-3 text-${props.mode==='light' ? 'dark' : 'light'}`}>
            ...
            <p><b>{text.trim(" ").split(/\s+/).filter( (e) => e !=="").length}</b> words and <b>{text.length}</b> characters</p>
            <p>It will take <b>{0.008 * text.trim(" ").split(/\s+/).filter( (e) => e !=="").length}</b> minutes to read the text.</p>
            <h2>Preview</h2>
